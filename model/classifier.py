...
class Classifier(LightningModule):
    ...
    def __init__(self, num_classes: int, backbone: str, finetune: bool,
        STEM-DFER_ckpt: Optional[str] = None,
        ...
        if finetune:
            if STEM-DFER_ckpt is None:
                self.model = STEM-DFER.from_online(backbone).encoder
            else:
                self.model = STEM-DFER.from_file(backbone, STEM-DFER_ckpt).encoder
        else:
            self.model = None
