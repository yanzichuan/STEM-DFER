...
from stem_pytorch.model.decoder import STEMDecoder
...
class STEM_DFER(LightningModule):
    ...
    def compute_multi_scale_frame_diff(self, x: Tensor) -> Tensor:
        """
        x: (B, C, T, H, W)
        Returns: (B, K*C, T, H, W) where K is number of scales.
        """
        b, c, t, h, w = x.shape
        diffs = []
        for s in self.motion_scales:
            # E_t = I_t - I_{t-s}
            # Pad with zeros or repeat first frame for t < s
    ...
    def forward(self, x, mask):
        # x is assumed to be raw frames
        # During pre-training, we learn features from appearance and motion targets
    ...
    def training_step(self, batch, batch_idx):
        x, mask = batch # x: (B, C, T, H, W)
        
        # 1. Compute targets
        # Appearance target (raw frames patched)
        y_app = self._patchify(x)
        
        # Motion target (multi-scale diffs patched)
        motion_x = self.compute_multi_scale_frame_diff(x)
        y_mot = self._patchify(motion_x)
        
        # 2. Filter masked patches
        b, _, _ = y_app.shape
        # ~mask because mask defines visible tokens 
        # (Wait, check STEMEncoder mask convention: mask True means visible)
        # Yes: emb = emb[mask].view(b, -1, c)
        # So targets should be the MASKED ones for reconstruction
        ...
        self.log("train_loss_app", loss_app)
        self.log("train_loss_mot", loss_mot)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_loss_app", loss_app, sync_dist=self.distributed)
        self.log("val_loss_mot", loss_mot, sync_dist=self.distributed)
        return loss

    def _patchify(self, x):
        """
        x: (B, C, T, H, W) --> (B, N, PatchPixelCount)
        """
        p = self.patch_size
        ts = self.tubelet_size
        # (B, C, T/ts, ts, H/p, p, W/p, p)
        y = x.unfold(2, ts, ts).unfold(3, p, p).unfold(4, p, p)
        # b c nt nh nw pt ph pw -> b (nt nh nw) (c pt ph pw)
