...
class STEMDecoder(STEMDecoder):
    """
    STEMDecoder with dual heads for Appearance and Motion reconstruction.
    """

    def __init__(self, *args, motion_channels=9, **kwargs):
        super().__init__(*args, **kwargs)
        # Appearance head is already in self.head (3 * tubelet * patch^2)
        # We need a motion head (motion_channels * tubelet * patch^2)
        output_dim_mot = motion_channels * self.tubelet_size * self.patch_size * self.patch_size
        self.motion_head = Linear(self.embed_dim, output_dim_mot)
    ...
    def forward_features(self, x, return_token_num=0):
        ...
        # Return both outputs
        app_out = self.head(x)
        mot_out = self.motion_head(x)
        return app_out, mot_out
    ...
    def forward(self, x, mask):
        ...
        app_out, mot_out = self.forward_features(x, return_token_num=mask_num)
        return app_out, mot_out
