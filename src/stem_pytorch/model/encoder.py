...
class STEMEncoder(STEMEncoder):
    """
    Spatio-Temporal Emotion Modeling Encoder (STEMEncoder).
    Supports Dynamic Expression Expert Module (DEEM) for Stage 2 adaptation (D3E).
    """

    def add_deem(self, layer_indices: Sequence[int], num_experts=8, top_r=2, reduction_dim=64):
        """
        Inject DEEM modules into specified blocks.
        """
        for idx in layer_indices:
            if 0 <= idx < len(self.blocks):
                from .modules import DEEM
                self.blocks[idx].deem = DEEM(
                    dim=self.embed_dim,
    ...
    def get_balancing_loss(self) -> Tensor:
        loss = torch.tensor(0., device=self.norm.weight.device)
        count = 0
        for block in self.blocks:
            if hasattr(block, 'deem') and block.deem is not None:
                loss += block.deem.get_balancing_loss()
                count += 1
