...
class Stem(Module):
    ...
    @classmethod
    def from_file(cls, model_name: str, path: str) -> "Stem":
        if path.endswith(".pt"):
    ...
    @classmethod
    def from_online(cls, model_name: str, full_model: bool = False) -> "Stem":
        config = resolve_config(model_name)
        ...
        if not file.exists():
            with DownloadProgressBar(unit="B", unit_scale=True, miniters=1, desc="Downloading Stem model") as pb:
    ...
    def clean_cache(cls, verbose: bool = True) -> None:
        ...
        if path.exists():
            shutil.rmtree(path)
            if verbose:
                print("Stem checkpoints cache cleaned.")
