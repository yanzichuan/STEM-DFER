...
from stem_pytorch import STEM-DFER


class TestSTEM-DFERPytorch:
    assertTrue: Callable
    ...
    def test_load_full_model_from_file(self):
        STEM-DFER.from_file(self.MODEL_NAME, self.MODEL_FULL_PATH)
        self.assertTrue(True)

    def test_load_encoder_from_file(self):
        STEM-DFER.from_file(self.MODEL_NAME, self.MODEL_ENCODER_PATH)
        self.assertTrue(True)

    def test_load_full_model_from_online(self):
        STEM-DFER.from_online(self.MODEL_NAME, full_model=True)
        self.assertTrue(True)

    def test_load_encoder_from_online(self):
        STEM-DFER.from_online(self.MODEL_NAME, full_model=False)
        self.assertTrue(True)
    ...
    def test_extract_wild_video(self):
        ...
        model = STEM-DFER.from_file(self.MODEL_NAME, self.MODEL_ENCODER_PATH)
        if self.USE_GPU:
            model.cuda()
    ...
    def test_reconstruct_clip(self):
        model = STEM-DFER.from_file(self.MODEL_NAME, self.MODEL_FULL_PATH)
        if self.USE_GPU:
            model.cuda()
